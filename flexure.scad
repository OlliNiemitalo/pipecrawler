...
// spiral flexure
module spiral_flexure(radii, angles, t_angles, t_in, extra_in_start, extra_in_end, t_out, extra_out_start, extra_out_end, n, skips, spring_n = $fn) {    
    points = [
        for(step = [skips[0]:spring_n-skips[1]]) [
        ...
        for(step = [skips[2]:spring_n-skips[3]]) [
