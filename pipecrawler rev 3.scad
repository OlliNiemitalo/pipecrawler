...
module spring(r, angles, t_angles, t_in, extra_in_start, extra_in_end, t_out, extra_out_start, extra_out_end, n, height, $fn = $fn) {    
    points = [
        for(step = [0:$fn]) [r * cos(angles[0] + (angles[1] - angles[0])*smooth(step/$fn)) + (t_out + (step < $fn/2? extra_out_start : extra_out_end)*ease(smooth(step/$fn), n)) * cos(t_angles[0] + (t_angles[1] - t_angles[0])*smooth(step/$fn)), r * sin(angles[0] + (angles[1] - angles[0])*smooth(step/$fn)) + (t_out + (step < $fn/2? extra_out_start : extra_out_end)*ease(smooth(step/$fn), n)) * sin(t_angles[0] + (t_angles[1] - t_angles[0])*smooth(step/$fn))],
        for(step = [0:$fn]) [r * cos(angles[1] + (angles[0] - angles[1])*smooth(step/$fn)) - (t_in + (step < $fn/2?extra_in_start : extra_in_end)*ease(smooth(step/$fn), n)) * cos(t_angles[1] + (t_angles[0] - t_angles[1])*smooth(step/$fn)), r * sin(angles[1] + (angles[0] - angles[1])*smooth(step/$fn)) - (t_in + (step < $fn/2?extra_in_start : extra_in_end)*ease(smooth(step/$fn), n)) * sin(t_angles[1] + (t_angles[0] - t_angles[1])*smooth(step/$fn))]
    ];
    linear_extrude(height) polygon(points);
}
...
for (i = [0:2]) rotate([0, 0, 360/3*i]) {
    difference() {
        union() {
            ...
            // Spring
            if (i < 2) {
                mirror([0, 1-i, 0]) translate([spring_shift_x, spring_shift_y, -wheel_bearing_with_washers_height/2 - wheel_vertical_clearance - wheel_support_height]) spring(spring_radius, spring_angles, spring_extra_angles, spring_t/2, 1, 1, spring_t/2, 1, 1, spring_n, wheel_support_height, $fn=64);
                translate([spring2_shift_x, spring2_shift_y, -wheel_bearing_with_washers_height/2 - wheel_vertical_clearance - wheel_support_height]) mirror([0, i, 0]) spring(spring2_radius, spring2_angles, spring2_extra_angles, spring2_t/2, 2, 1, spring2_t/2, 1, 0, spring2_n, wheel_support_height, $fn=64);
            }
...
// Spring support
    difference() {
        translate([0, 0, -wheel_bearing_with_washers_height/2 - wheel_support_height - wheel_vertical_clearance]) linear_extrude(wheel_support_height) polygon([[cos(60 + spring_support_angle)*body_od/2*0.996, sin(60 + spring_support_angle)*body_od/2*0.996], [cos(60 - spring_support_angle)*body_od/2*0.996, sin(60 - spring_support_angle)*body_od/2*0.996], [cos(60)*(pipe_id/2 - wheel_od/2 + wheel_tight), sin(60)*(pipe_id/2 - wheel_od/2 + wheel_tight)]]);
        translate([cos(180 + 60)*support_t, sin(180+60)*support_t, -wheel_bearing_with_washers_height/2 - wheel_support_height - wheel_vertical_clearance]) linear_extrude(wheel_support_height) polygon([[cos(60 + spring_support_angle)*body_od/2*0.996, sin(60 + spring_support_angle)*body_od/2*0.996], [cos(60 - spring_support_angle)*body_od/2*0.996, sin(60 - spring_support_angle)*body_od/2*0.996], [cos(60)*(pipe_id/2 - wheel_od/2 + wheel_tight), sin(60)*(pipe_id/2 - wheel_od/2 + wheel_tight)]]);
    }
// Fixed wheel support
            difference() {
        translate([0, 0, -wheel_bearing_with_washers_height/2 - wheel_support_height - wheel_vertical_clearance]) linear_extrude(wheel_support_height) polygon([[cos(180 + 60 + spring_support_angle)*body_od/2*0.996, sin(180 + 60 + spring_support_angle)*body_od/2*0.996], [cos(180 + 60 - spring_support_angle)*body_od/2*0.996, sin(180 + 60 - spring_support_angle)*body_od/2*0.996], [cos(180 + 60)*(pipe_id/2 - wheel_od/2 + wheel_tight + wheel_support_width/2), sin(180 + 60)*(pipe_id/2 - wheel_od/2 + wheel_tight + wheel_support_width/2)]]);
        translate([-cos(180 + 60)*support_t, -sin(180+60)*support_t, -wheel_bearing_with_washers_height/2 - wheel_support_height - wheel_vertical_clearance]) linear_extrude(wheel_support_height) polygon([[cos(180 + 60 + spring_support_angle)*body_od/2*0.996, sin(180 + 60 + spring_support_angle)*body_od/2*0.996], [cos(180 + 60 - spring_support_angle)*body_od/2*0.996, sin(180 + 60 - spring_support_angle)*body_od/2*0.996], [cos(180 + 60)*(pipe_id/2 - wheel_od/2 + wheel_tight + wheel_support_width/2), sin(180 + 60)*(pipe_id/2 - wheel_od/2 + wheel_tight + wheel_support_width/2)]]);
            }
        }
